...
from utils.clustering.auxiliary_functions import generate_overlapping_clusters, make_blobs
...
NN2 = NearestNeighbors(n_neighbors=np.log(X_res.size).astype(int)).fit(X_res)
distances2, indices2 = NN2.kneighbors(X_res)

print(distances1)

plt4.set_title('minPts elbow')
plt4.plot(np.sort(distances1[:, distances1.shape[1]-1]), color='red', label = 'NoUndersample')
plt4.plot(np.sort(distances2[:, distances2.shape[1]-1]), color='green', label='Undersampled')
plt4.legend()
